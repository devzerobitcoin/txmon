...
// The number of peers to pull from the leaderboard and connect to.
var size = 50;
...
request('https://bitnodes.21.co/api/v1/nodes/leaderboard/?limit=' + size + '&page=1',
  {json:true},
  function(error, response, body) {

    async.reduce(body.results, [], function(memo, item, callback) {
      // TODO: Handle ipv6 addresses
      memo.push({ ip : { v4: item.node.split(':')[0] }});
      callback(null, memo);

    }, function(err, result){

      // Add specific peers to connect to.
      var peers = result.concat(myPeers);
      console.log(result);

      var pool = new Pool({
        network: Networks.livenet,
        dnsSeed: false,
        maxSize: peers.length,
        addrs: peers
      });
